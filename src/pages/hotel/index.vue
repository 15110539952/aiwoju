  ...
  <div class="hotel">
    ...
    <EasyRefresh
      ...
      <div class="address-block"
           @click="$router.push({path:'/map',query:{lng:hotel.lng,lat:hotel.lat,name:hotel.name,address:hotel.address}})">
        <div class="left">
      ...
      <div class="room-list">
        <div class="room-item"
          ...
          <div class="price-item van-hairline--top"
            ...
            <div class="fix">
              <van-button type="primary" size="large" @click.stop="order(room.id)" :disabled="item.room_rest_num < 1">{{
                                                                                                                      item.room_rest_num
                                                                                                                      <
                                                                                                                      1?'已满房':'在线预定'
                                                                                                                      }}
      ...
      <div class="facilities">
        <div class="title-one">配套设施<p class="title-right" @click="$router.push('/hotelDetail')">设施详情 <i
          class="iconfont iconarrow-right"></i></p></div>
      ...
      <div class="facilities">
        <div class="title-one">
          房间评价<p class="title-right" @click="$router.push('/evaluateList')">更多评价 <i
          class="iconfont iconarrow-right"></i></p>
        </div>
        ...
        <div class="evaluate-item" v-for="(item,comindex) in comment" :key="'com_'+comindex">
          ...
          <div class="content">
            <div class="one">
              <p>{{item.user_id.length>0?item.user_id[0].nickname:''}}</p>
              <p>{{item.room_type_id}}</p>
            </div>
            <div class="score-box">
              <i class="iconfont iconpinglun2"
            ...
            <div class="time-block">
              <p><span>{{item.start_time}}</span>入住</p>
              <p><span>{{item.creatime}}</span> 发表</p>
            </div>
    ...
    <van-action-sheet v-model="roomDetailShow"
      ...
      <div class="roomDetail" v-if="roomDetail">
        ...
        <van-button class="footer" type="primary" size="large"
                    @click="roomDetailShow=false">查看更多价格
        </van-button>
      </div>
    </van-action-sheet>
